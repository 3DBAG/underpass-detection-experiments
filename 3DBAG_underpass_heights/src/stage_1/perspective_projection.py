"""
Project 2D camera images onto 3D building facades so that clean 2D facade images can be extracted from a 3D city model.
Determines which part of a 3D wall is visible in a given image and maps image pixels onto that wall.
...
def get_offset(img_id):
    ...
    """
    offset_file = os.path.join(PROJECT_ROOT, 'import', 'offset.xyz')
...
def get_camera_parameters(img_id):
    ...
    """
    cp_file = os.path.join(PROJECT_ROOT, 'import', 'calibrated_camera_parameters.txt')
...
def draw_points(points, img_id):
    ...
    result_file = os.path.join(PROJECT_ROOT, 'result', 'wrong_example.jpg')
...
if __name__ == '__main__':
    ...
    img_id = '405_0031_00130737.tif'
    # project_3D(img_id)  # This function has incomplete logic - skipping for now
