...
# Set matplotlib backend before importing pyplot to avoid Qt plugin issues
import matplotlib
matplotlib.use('Agg')
...
# Project root is two levels up from src/stage1/
PROJECT_ROOT = os.path.dirname(os.path.dirname(SCRIPT_DIR))

# Environment variable to prevent Qt platform issues
os.environ['QT_QPA_PLATFORM'] = 'offscreen'

"""
return facade list (Wallsurface) [[[point1],[point2],[point3],[point4]],[...],[....]] with real coordinates in certain LOD

It takes 3D BAG building geometry, merges fragmented wall surfaces into clean façade planes,
projects façade images, detects windows/doors in 2D, 
and reconstructs them back into 3D CityJSON geometry (walls, windows, doors, connecting surfaces).
...
if __name__ == '__main__':
    ...
    # image = cv2.imread(os.path.join(PROJECT_ROOT, 'image', img_id))
    ...
    # ========================================================================
    # PHASE 2: Interactive Ground Truth Registration
    # ========================================================================
    # Load the source image and overlay projected facades
    image = cv2.imread(os.path.join(PROJECT_ROOT, 'image', img_id))
    # Draw projected rectangles in blue (RGB 0,0,255) for reference
    for rectangle in pts_original:
        cv2.polylines(image, [rectangle], True, (0, 0, 255), 2)

    # Save original image for reset functionality
    clone = image.copy()
    
    # Create interactive window for manual point selection
    # Disabled for headless/non-GUI environments
    # cv2.namedWindow("image")
    # cv2.setMouseCallback("image", draw_rectangle)

    # Interactive loop: User selects ROI (Region of Interest) by clicking points
    # Instructions:
    #   - Click to select two opposite corners of a rectangle
    #   - Press 'r' to reset and reselect
    #   - Press 'c' when done selecting
    # Disabled for non-interactive mode. Using default roi_points instead.
    # while True:
    #     cv2.imshow("image", image)
    ...
    #
    #     elif key == ord("c"):
    #         # Complete: finish manual selection
    #         print("end selecting")
    #         break
    
    # For non-interactive mode, use default ROI points or load from file
    print(f"ROI Points (using default): {roi_points}")
    ...
    # ========================================================================
    # PHASE 3: Least Squares Regression (LSR) - Calibrate projection
    # ========================================================================
    # Compute linear regression between projected points and ground truth
    # This generates transformation coefficients to correct systematic errors
    # Output: slope_x, intercept_x, slope_y, intercept_y
    #   corrected_x = slope_x * projected_x + intercept_x
    #   corrected_y = slope_y * projected_y + intercept_y
    
    if len(roi_points) > 0:
        # If ROI points were collected, compute regression
        slope_x, intercept_x, slope_y, intercept_y = LSR(roi_points)
    else:
        # Non-interactive mode: use identity transformation (no correction)
        slope_x, intercept_x, slope_y, intercept_y = 1.0, 0.0, 1.0, 0.0
        print("Warning: No ROI points collected. Using identity transformation (no correction).")

    # ========================================================================
    # PHASE 4: Apply registration to all facades
    # ========================================================================
    # Apply computed regression coefficients to correct all projected points
    for rectangle in rectangles:
        rect = rectangle.copy()

        # Step 1: Project 3D facade to 2D
        new_facede_2d = projection(img_id, rect)

        # Store original projection before correction
        pts_original.append(np.array(new_facede_2d))

        # Step 2: Apply registration correction using regression coefficients
        # This corrects systematic projection errors and camera misalignment
        new_facede_2d = registration(new_facede_2d, slope_x, intercept_x, slope_y, intercept_y)

        # Convert to integer pixel coordinates
        pts = np.array(new_facede_2d, np.int32)

        # Generate output filename for rectified texture
        name = os.path.join(PROJECT_ROOT, 'texture', 'texture_' + img_id.strip().split(".")[0] + '_' + str(id_count) + '.jpg')
        img_list.append(name)
        
        # Store corrected points for visualization and output
        pts_groundtruthlist.append(pts)

        # TODO: Extract and save rectified texture image
        # perspective(img_id, rectangle, pts, name)

        id_count += 1

    # ========================================================================
    # PHASE 5: Visualization - Display final corrected facades
    # ========================================================================
    # Load image and overlay corrected facade projections for verification
    image = cv2.imread(os.path.join(PROJECT_ROOT, 'image', img_id))
    
    # Draw corrected rectangles in red (RGB 255,0,0) on the image
    for rectangle in pts_groundtruthlist:
        cv2.polylines(image, [rectangle], True, (255, 0, 0), 5)
    
    # Display result for user verification
    # Disabled for non-GUI environments - save to file instead
    output_viz_file = os.path.join(PROJECT_ROOT, 'result', 'corrected_facades.jpg')
    os.makedirs(os.path.dirname(output_viz_file), exist_ok=True)
    cv2.imwrite(output_viz_file, image)
    print(f"Visualization saved to: {output_viz_file}")
    ...
    # ========================================================================
    # PHASE 6: Save Registration Model
    # ========================================================================
    # Export regression coefficients for future use on other images
    # File format: image_type slope_x intercept_x slope_y intercept_y
    result_file = os.path.join(PROJECT_ROOT, 'result', 'registration.txt')
